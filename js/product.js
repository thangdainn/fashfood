    ...
    var html = `
        <div class="col l-3 m-4 c-6">
            <div class="product_item">
                <a href="index.html?${product.category.replaceAll(' ', '-').toLowerCase()}?${tmpName}" class="product_item-link">
                    <img src = "${product.img}" class = "product_item-img"></img>
                    <h3 class="product_item-name">${product.name}</h3>
                    <div class="product_item-price">
                        <p class="product_item-current-price">${product.currentPrice}</p>
                        <p class="product_item-old-price">${product.oldPrice}</p>
                    </div>
                </a>
            </div>
        </div>
...
function showCurrentNavbar(str) {
    var currentNavbar = document.querySelectorAll('.header_navbar-item-link');
    for (var i = 0; i < currentNavbar.length; i++) {
        currentNavbar[i].classList.remove('header_navbar-item-link--active');
    }
    for (var i = 0; i < currentNavbar.length; i++) {
        if (currentNavbar[i].getAttribute('data-value').toLowerCase() == str.replaceAll('-', '-').toLowerCase()) {
            currentNavbar[i].classList.add('header_navbar-item-link--active');
            break;
        }
    }
}
...
function showFilter(category) {
    ...
    var s = `
        <li class="product_filter-item">
            <button class="product_filter-item-btn product_filter-item-btn--active" onclick="showProduct(1)">Tất cả</button>
        </li>
    `;
    ...
    var html = filterArray.map(function(filter) {
        return `
            <li class="product_filter-item">
                <button class="product_filter-item-btn" onclick="showFilterProduct('${category}', '${filter}', 1)">${filter}</button>
            </li>
        `;
    });
    s = s + html.join('');
...
function showFilterProduct(category, filterName, start) {
    ...
    var filter = document.querySelectorAll('.product_filter-item-btn');
    var arr;
...
function showProduct(start) {
    ...
    document.querySelector('.product_logo-name').innerHTML = category;
    document.getElementById('show-product').innerHTML = arr.join('');
    if (category != 'Tất cả'){
        document.querySelector('.product_header').scrollIntoView();
    }
...
function showProductDetail() {
    ...
    var html = `
        <div class="col l-6 m-12 c-12">
            <div class="product_detail-img-box">
                <img src="${detailProduct.img}" alt="" class="product_detail-img">
            </div>
        </div>
        <div class="col l-6 m-12 c-12">
            <div class="product_detail-info">
                <span class="product_detail-name">${detailProduct.name}</span>
                <div class="product_detail-price">
                    <p class="product_detail-current-price">${detailProduct.currentPrice}</p>
                    <p class="product_detail-old-price">${detailProduct.oldPrice}</p>
                </div>
                <div class="product_detail-policy">
                    <div class="product_detail-policy-item">
                        <i class="uil uil-box"></i>
                        <span class="product_detail-policy-text">Bộ sản phẩm gồm: Hộp, Sách hướng dẫn, Cây lấy sim, Cáp Lightning - Type C</span>
                    </div>
                    <div class="product_detail-policy-item">
                        <i class="uil uil-sync"></i>
                        <span class="product_detail-policy-text">Hư gì đổi nấy trong 12 tháng</span>
                    </div>
                    <div class="product_detail-policy-item">
                        <i class="uil uil-shield-check"></i>
                        <span class="product_detail-policy-text">Bảo hành chính hãng 2 năm</span>
                    </div>
                    <div class="product_detail-policy-item">
                        <i class="uil uil-truck"></i>
                        <span class="product_detail-policy-text">Giao hàng nhanh toàn quốc</span>
                    </div>
                    <div class="product_detail-policy-item">
                        <i class="uil uil-phone"></i>
                        <span class="product_detail-policy-text">
                            Tổng đài:
                            <a href="tel:0976124506" class="product_detail-phone">0976124506</a>                                      
                        </span>
                    </div>
                </div>
                <div class="product_detail-pay">
                    <button class="product_detail-add-cart">Thêm vào giỏ</button>
                    <button class="product_detail-purchase">Mua ngay</button>
                </div>
            </div>
        </div>
    `;
    ...
    //Xử lý purchase, addcart
    haveToLogin();
    addToCart();
}

function haveToLogin() {
    var purchaseBtn = document.querySelector('.product_detail-purchase');
