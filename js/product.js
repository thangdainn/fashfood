...
if (!products) {
    products = [
        //chickens
        {id: 'chicken001', category: 'chicken', name: '1 miếng Gà Rán', img: './img/product/chickens/x3pcs_chicken.png', currentPrice: '37.000', oldPrice: '', detailCategory: 'Đùi gà'},
...
function htmlProduct(product) {
    var tmpName = product.name.replaceAll(' ', '-'); // Thay dấu cách bằng dấu gạch ngang
    var html = `
        <div class="col l-3 m-4 c-6">
            <div class="product__item">
                <a href="index.html?name=${tmpName}" class="product__item-link"> <!-- Thêm tham số name vào URL -->
                    <img src = "${product.img}" class = "product__item-img"></img>
                    <h3 class="product__item-name">${product.name}</h3>
                    <div class="product__item-price">
                        <p class="product__item-current-price">${product.currentPrice}đ</p>
...
//Product Detail
//Khi nhấn vào sản phẩm hiện thông tin chi tiết 
function showProductDetail() {
    document.querySelector('.cart').style.display = 'none';
    document.querySelector('.order').style.display = 'none';

    var url = window.location.href;
    var queryParams = new URLSearchParams(url.split('?')[1]); // Lấy query string
    var productName = queryParams.get('name'); // Lấy tham số 'name'

    if (!productName) {
        console.error("Tên sản phẩm không có trong URL.");
        return;
    }

    // Chuyển dấu gạch ngang ngược lại thành dấu cách và giải mã URL
    productName = decodeURIComponent(productName).replaceAll('-', ' ');
    ...
    if (!detailProduct) {
        console.error("Không tìm thấy sản phẩm với tên này.");
        return;
    }

    // Tạo HTML để hiển thị chi tiết sản phẩm
    var html = `
            ...
            <div class="product__detail-info">
                ...
                <div class="product__detail-policy">
                    <div class="product__detail-policy-item">
                        ...
                        <span class="product__detail-policy-text">Bao gồm: Hộp đựng thức ăn, muỗng đũa dùng 1 lần</span>
                    </div>
                    <div class="product__detail-policy-item">
                        <i class="uil uil-sync"></i>
                        <span class="product__detail-policy-text">Hỗ trợ đổi món trong vòng 30 phút nếu có sai sót</span>
                    </div>
                    <div class="product__detail-policy-item">
                        <i class="uil uil-shield-check"></i>
                        <span class="product__detail-policy-text">Đảm bảo vệ sinh an toàn thực phẩm</span>
                    </div>
                    <div class="product__detail-policy-item">
                ...
                <div class="product__detail-pay">
                    <button class="product__detail-add-cart" id="add-to-cart">Thêm vào giỏ</button>
                    <button class="product__detail-buy" id="buy-now">Mua ngay</button>
                </div>
            </div>
        </div>
    `;
    ...
    document.getElementById('show-product-detail').innerHTML = html; // Hiển thị chi tiết sản phẩm

    // Gắn sự kiện cho các nút sau khi hiển thị chi tiết sản phẩm
    document.getElementById('add-to-cart').addEventListener('click', function() {
        addToCart(detailProduct); // Thêm sản phẩm vào giỏ
    });

    document.getElementById('buy-now').addEventListener('click', function() {
        haveToLogin(); // Xử lý hành động khi người dùng mua ngay
    });
}
...
function haveToLogin(product) {
    ...
    if (notUser.style.display == 'block') {
        ...
        getCurrentProduct(product); // Lưu trữ thông tin sản phẩm hiện tại
        window.location.href = 'index.html?cart'; // Chuyển hướng đến trang giỏ hàng
    }
}
